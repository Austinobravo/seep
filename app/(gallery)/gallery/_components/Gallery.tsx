        ...
        {Images.map((image, index)=> (
            <div key={index} className='space-y-5 py-5'>
                <FadeInSection direction={`up`}>
                    <Image src={image.first} width={200} height={100} alt='image' className='w-full rounded-lg cursor-pointer' onClick={()=> {setIsModalOpen(!isModalOpen), setCurrentImage(image.first)}}/>
                </FadeInSection>
                <div className='flex md:flex-nowrap flex-wrap gap-5'>
                    <FadeInSection direction={`left`}>
                        <Image src={image.second} width={200} height={100} alt='image' className='w-full rounded-lg cursor-pointer md:h-[28rem] h-full' onClick={()=> {setIsModalOpen(!isModalOpen), setCurrentImage(image.second)}}/>
                    </FadeInSection>
                    <FadeInSection direction={`right`}>
                        <Image src={image.third} width={200} height={100} alt='image' className='w-full rounded-lg cursor-pointer md:h-[28rem] h-full' onClick={()=> {setIsModalOpen(!isModalOpen), setCurrentImage(image.third)}}/>
                    </FadeInSection>
                </div>
                {isModalOpen && 
